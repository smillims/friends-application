...
fetch("https://randomuser.me/api/?results=10")
  ...
  .then((users) => {
    ...
    document.addEventListener("click", (event) => filterSetting(event, users.results));
  });
...
function filterSetting(event, friends) {
  ...
  if (event.target.closest(".filter_menu")) sortFriends(event.target, friends);
}
...
function filterByGender(friends) {
  const checkboxes = Array.from(document.querySelectorAll(".gender_checkbox:checked"));
